...
if __name__ == "__main__":
    ...
    for line in f_input:
        ...
        tfile = line_split[0]
        label = int(line_split[1]) #negative and positive sample
        ...
        if tfile == "train":
            ...
            fo = f_test
        if user_id != last_user_id: #if new user
            movie_id_list = []
            cate_list = []
            dt_list = []
        else: #if old user
            history_clk_num = len(movie_id_list) #=1
            cat_str = "" #category
            mid_str = "" #movie id
            dt_str = "" #date time
            for c1 in cate_list:
        ...
        if label: #check xem co phai positive sample k
