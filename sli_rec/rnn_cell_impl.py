...
class CARNNCell(RNNCell):
  def __init__(self, num_units, activation=None, reuse=None):
    super(CARNNCell, self).__init__(_reuse=reuse)
    self._num_units = num_units
    self._activation = activation or math_ops.tanh
    self._linear = None
    self._time_kernel_w = None
    self._time_kernel_b = None
  ...
  def call(self, inputs, state):
    time_score = tf.expand_dims(inputs[:,-1],-1)
    inputs = inputs[:,:-1]
    dtype = inputs.dtype
    if self._time_kernel_w is None:
      scope = vs.get_variable_scope()
      with vs.variable_scope(scope) as unit_scope:
        with vs.variable_scope(unit_scope):
          self._time_kernel_w=vs.get_variable(
              "_time_kernel_w",shape=[self._num_units],dtype=dtype)
          self._time_kernel_b=vs.get_variable(
              "_time_kernel_b",shape=[self._num_units],dtype=dtype)
    time_vec=self._activation(time_score*self._time_kernel_w+self._time_kernel_b)
    state=time_vec*state
...
class Time1LSTMCell(RNNCell):
  ...
  def __init__(self, num_units,
    ...
    super(Time1LSTMCell, self).__init__(_reuse=reuse)
    if not state_is_tuple:
      logging.warn("%s: Using a concatenated state is slower and will soon be "
                   "deprecated.  Use state_is_tuple=True.", self)
    if num_unit_shards is not None or num_proj_shards is not None:
    ...
    self._linear2 = None
    self._time_kernel_w = None
    self._time_kernel_t = None
    self._time_bias = None
    self._o_kernel_t = None
    if self._use_peepholes:
      self._w_f_diag = None
      self._w_i_diag = None
      self._w_o_diag = None
  ...
  def call(self, inputs, state):
    ...
    if self._time_kernel_w is None:
      ...
      with vs.variable_scope(
        ...
        with vs.variable_scope(unit_scope):
          self._time_kernel_w = vs.get_variable(
              "_time_kernel_w", shape=[input_size, self._num_units], dtype=dtype)
          self._time_kernel_t = vs.get_variable(
              "_time_kernel_t", shape=[self._num_units], dtype=dtype)
          self._time_bias = vs.get_variable(
              "_time_bias", shape=[self._num_units], dtype=dtype)
          self._o_kernel_t = vs.get_variable(
              "_o_kernel_t", shape=[self._num_units], dtype=dtype)    
    time_state = math_ops.matmul(inputs, self._time_kernel_w) + sigmoid(time_score * self._time_kernel_t) + self._time_bias       
    ot_state = time_score * self._o_kernel_t   
    ...
    if self._use_peepholes:
      c = (sigmoid(f + self._forget_bias + self._w_f_diag * c_prev) * c_prev +
           sigmoid(i + self._w_i_diag * c_prev) * sigmoid(time_state) * self._activation(j))
    else:
      c = (sigmoid(f + self._forget_bias) * c_prev + sigmoid(i) * sigmoid(time_state) * self._activation(j))
...
class Time2LSTMCell(RNNCell):
  ...
  def __init__(self, num_units,
    ...
    super(Time2LSTMCell, self).__init__(_reuse=reuse)
    if not state_is_tuple:
      logging.warn("%s: Using a concatenated state is slower and will soon be "
                   "deprecated.  Use state_is_tuple=True.", self)
    if num_unit_shards is not None or num_proj_shards is not None:
  ...
  def call(self, inputs, state):
      ...
      with vs.variable_scope(
        ...
        with vs.variable_scope(unit_scope):
          ...
          self._time_kernel_t1 = vs.get_variable(
              "_time_kernel_t1", shape=[self._num_units], dtype=dtype)
          ...
          self._time_kernel_t2 = vs.get_variable(
              "_time_kernel_t2", shape=[self._num_units], dtype=dtype)
          self._time_bias2 = vs.get_variable(
              "_time_bias2", shape=[self._num_units], dtype=dtype)
          self._o_kernel_t = vs.get_variable(
              "_o_kernel_t", shape=[self._num_units], dtype=dtype)    
    time_state1 = math_ops.matmul(inputs, self._time_kernel_w1) + sigmoid(time_score * self._time_kernel_t1) + self._time_bias1  
    time_state2 = math_ops.matmul(inputs, self._time_kernel_w2) + sigmoid(time_score * self._time_kernel_t2) + self._time_bias2      
    ot_state = time_score * self._o_kernel_t   
    ...
    if self._use_peepholes:
      c1 = (sigmoid(f + self._forget_bias + self._w_f_diag * c_prev) * c_prev +
           sigmoid(i + self._w_i_diag * c_prev) * sigmoid(time_state1) * self._activation(j))
      c = (sigmoid(f + self._forget_bias + self._w_f_diag * c_prev) * c_prev +
           sigmoid(i + self._w_i_diag * c_prev) * sigmoid(time_state2) * self._activation(j))
    else:
      c1 = (sigmoid(f + self._forget_bias) * c_prev + sigmoid(i) * sigmoid(time_state1) * self._activation(j))
      c = (sigmoid(f + self._forget_bias) * c_prev + sigmoid(i) * sigmoid(time_state2) * self._activation(j))
    ...
    if self._cell_clip is not None:
      ...
      # pylint: enable=invalid-unary-operand-type
    if self._use_peepholes:
      m = sigmoid(o + self._w_o_diag * c1) * self._activation(c1)
    else:
      m = sigmoid(o) * self._activation(c1)
...
class Time3LSTMCell(RNNCell):
  ...
  def __init__(self, num_units,
    ...
    super(Time3LSTMCell, self).__init__(_reuse=reuse)
    if not state_is_tuple:
      logging.warn("%s: Using a concatenated state is slower and will soon be "
                   "deprecated.  Use state_is_tuple=True.", self)
    if num_unit_shards is not None or num_proj_shards is not None:
  ...
  def call(self, inputs, state):
      ...
      with vs.variable_scope(
        ...
        if self._num_unit_shards is not None:
          unit_scope.set_partitioner(
              partitioned_variables.fixed_size_partitioner(
                  self._num_unit_shards))
        self._linear1 = _Linear([inputs, m_prev], 3 * self._num_units, True)

    # i = input_gate, j = new_input, f = forget_gate, o = output_gate
    lstm_matrix = self._linear1([inputs, m_prev])
    i, j, o = array_ops.split(
        value=lstm_matrix, num_or_size_splits=3, axis=1)
    o = o + ot_state  
    # Diagonal connections
    if self._use_peepholes and not self._w_f_diag:
      scope = vs.get_variable_scope()
    ...
    if self._use_peepholes:
      c1 = (1. - sigmoid(i + self._w_i_diag * c_prev) * sigmoid(time_state1)) * c_prev + sigmoid(i + self._w_i_diag * c_prev) * sigmoid(time_state1) * self._activation(j)
      c = (1. - sigmoid(i + self._w_i_diag * c_prev)) * c_prev + sigmoid(i + self._w_i_diag * c_prev) * sigmoid(time_state2) * self._activation(j)
    else:
      c1 = (1. - sigmoid(i) * sigmoid(time_state1)) * c_prev + sigmoid(i) * sigmoid(time_state1) * self._activation(j)
      c = (1. - sigmoid(i)) * c_prev + sigmoid(i) * sigmoid(time_state2) * self._activation(j)
...
class Time4jLSTMCell(RNNCell):
  ...
  def __init__(self, num_units,
    ...
    super(Time4jLSTMCell, self).__init__(_reuse=reuse)
    if not state_is_tuple:
      logging.warn("%s: Using a concatenated state is slower and will soon be "
                   "deprecated.  Use state_is_tuple=True.", self)
    if num_unit_shards is not None or num_proj_shards is not None:
  ...
  def call(self, inputs, state):
    ...
    if self._time_kernel_w1 is None:
      ...
      with vs.variable_scope(
        ...
        with vs.variable_scope(unit_scope):
          self._time_kernel_w1 = vs.get_variable(
              "_time_kernel_w1", shape=[self._num_units, self._num_units], dtype=dtype)
          self._time_kernel_t1 = vs.get_variable(
              "_time_kernel_t1", shape=[self._num_units], dtype=dtype)
          self._time_bias1 = vs.get_variable(
              "_time_bias1", shape=[self._num_units], dtype=dtype)
          self._time_kernel_w2 = vs.get_variable(
              "_time_kernel_w2", shape=[self._num_units, self._num_units], dtype=dtype)
          self._time_kernel_t2 = vs.get_variable(
              "_time_kernel_t2", shape=[self._num_units], dtype=dtype)
          self._time_bias2 = vs.get_variable(
              "_time_bias2", shape=[self._num_units], dtype=dtype)
          ...
          self._o_kernel_t2 = vs.get_variable(
              "_o_kernel_t2", shape=[self._num_units, self._num_units], dtype=dtype)    
    time_now_state = tf.nn.tanh(time_now_score * self._time_kernel_t1 + self._time_bias1)
    time_last_state = tf.nn.tanh(time_last_score * self._time_kernel_t2 + self._time_bias2)
    ...
    i, j, f, o = array_ops.split(
        value=lstm_matrix, num_or_size_splits=4, axis=1)
    f = f + math_ops.matmul(time_last_state, self._time_kernel_w2)
    i = i + math_ops.matmul(time_now_state, self._time_kernel_w1)
    o = o + math_ops.matmul(time_now_state, self._o_kernel_t1) + math_ops.matmul(time_last_state, self._o_kernel_t2)   
    # Diagonal connections
    if self._use_peepholes and not self._w_f_diag:
      scope = vs.get_variable_scope()
      with vs.variable_scope(
  ...
  def call(self, inputs, state):
    ...
    if self._num_proj is not None:
      ...
      if self._proj_clip is not None:
        ...
        m = clip_ops.clip_by_value(m, -self._proj_clip, self._proj_clip)
        # pylint: enable=invalid-unary-operand-type
    c = att_score * c + (1. - att_score) * c
    m = att_score * m + (1. - att_score) * m
    new_state = (LSTMStateTuple(c, m) if self._state_is_tuple else
                 array_ops.concat([c, m], 1))
    return m, new_state
